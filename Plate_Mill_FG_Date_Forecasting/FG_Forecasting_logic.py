...
def predict_days(xgb_model, future_df):
...
def added_logics(predicted_days, supply_condition, tpip, thickness, buffer_cutting, buffer_htc, start_date):
    ...
    if start_date:
        approximate_completion_date = start_date + timedelta(days=int(predicted_days))
    else:
        approximate_completion_date = date.today() + timedelta(days=int(predicted_days))

    return predicted_days, approximate_completion_date
